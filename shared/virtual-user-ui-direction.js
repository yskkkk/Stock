...
export const VU_UI_DIRECTION_MUST = [
  ...
  "UI 수정 시 **PC(넓은 뷰포트)와 모바일(좁은 폭·터치)을 항상 함께** 고려해 동시에 맞춘다. 한쪽만 고치고 끝내지 않는다. 피드백마다 PC·모바일 검증 결과를 남긴다.",
  "단순 통일 금지: 같은 마크업이라도 레이아웃·간격·줄바꿈·터치 영역은 미디어쿼리/기존 반응형 패턴으로 **환경에 맞게** 조정한다. PC용을 모바일에 억지로 끼우거나, 모바일 한 열을 PC에 그대로 복제하지 않는다.",
  "모바일은 패딩·줄바꿈·overflow·터치(**최소 ~44×44px**)·safe-area·모달 footer wrap·폰트/간격만 기존 컴포넌트 안에서 조정한다. PC는 3열·여백·가독성·호버/클릭 밀도를 깨지 않는다.",
  "아이콘·마크·탭 아이콘·레일 아이콘은 **PC·모바일 모두**에서 크기·여백·대비를 깐깐히 본다. 너무 작거나(대략 표시 한 변 <16px 또는 터치 영역 <44px)·잘리거나·겹치거나·테마에서 사라지면 즉시 피드백한다.",
  "브랜드(YS)·거래소·종목 로고는 라이트/다크에서 선명해야 한다. 흐릿·투명 실패·흰 사각 매트·과도한 축소는 허용하지 않는다.",
  ...
  "UI/UX 피드백은 **한 번 하고 끝내지 않는다**. 아이콘 크기·터치·간격·정렬·가독성·PC↔모바일 괴리를 세션마다 계속 찾아 남긴다.",
];
...
export const VU_UI_DIRECTION_MUST_NOT = [
  ...
  "아이콘·칩·탭을 손가락/클릭하기 어렵게 축소하거나, PC에서만 맞고 모바일에서 깨지게(또는 그 반대로) 두지 않는다.",
  "PC·모바일을 ‘한 레이아웃으로 퉁치기’만 하고 한쪽이 깨지거나 과도한 여백/밀집이 생기게 두지 않는다.",
  "아이콘 크기·터치·정렬 문제를 ‘나중’으로 미루거나 한 기기만 보고 완료 처리하지 않는다.",
];
...
export function buildVuUiDirectionPromptBlock() {
  return [
    ...
    "가상 사용자는 **PC와 모바일을 둘 다** 깐깐하게 보고, 아이콘·터치·간격 등 UI/UX를 **계속** 피드백한다.",
    "",
    "### 해야 할 것",
...
export function vuUiDirectionSuggestionGuard(suggestion) {
  ...
  const guard =
    "기존 UI 패턴·골격 유지(좌측 열/3열 금지 변경). PC·모바일을 동시에 맞추되 단순 통일 금지·아이콘/터치 크기까지 깐깐히. 단축 UI는 관련 섹션에만 정렬·겹침 없이. 최소 diff로 해당 화면만.";
