...
export const ko = {
  ...
  access: {
    ...
    adminConfirm: "확인",
    adminIntro: "허용된 IP 또는 등록된 단말 MAC 중 하나라도 일치하면 API·대시보드를 쓸 수 있습니다. 대기: 접속 신청만 접수된 상태입니다. 승인하면 해당 IP(및 신청 시 제출한 MAC)가 아래 허용 목록에 반영되고, 거절하면 이번 신청만 종료됩니다. 허가 취소는 이미 허용된 IP 행을 목록에서 빼서 곧바로 API 접속을 막습니다. Wi-Fi MAC은 휴대폰 설정 등에서 확인해 접근 게이트에 저장할 수 있으며, 웹에서는 하드웨어 MAC을 자동으로 읽을 수 없습니다.",
    ...
    requestDeviceNote: "접근 게이트에서 Wi-Fi MAC을 저장·제출하면 IP가 바뀌어도 동일 단말로 인식됩니다.",
  },
  launch: {
    loading: "불러오는 중…",
  },
