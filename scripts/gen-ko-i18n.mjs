...
function extractPreservedKoSections(existingSrc) {
  const start = existingSrc.indexOf("  app: {");
  const end = existingSrc.indexOf("  errors: {");
  if (start === -1 || end === -1 || end <= start) return "";
  return existingSrc.slice(start, end);
}

const existingKo = fs.existsSync(out) ? fs.readFileSync(out, "utf8") : "";
const preservedKoSections = extractPreservedKoSections(existingKo);
if (!preservedKoSections) {
  console.error(
    "gen-ko-i18n: ko.ts에 app/errors 블록이 없어 보존 구간을 찾지 못했습니다. ko.ts를 먼저 복구한 뒤 다시 실행하세요.",
  );
  process.exit(1);
}
