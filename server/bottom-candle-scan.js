...
import { detectBottomCandleLatest } from "./bottom-candle-detect.js";
...
const STATE_FILE = "bottom-candle-scan-state.json";

const BATCH_SIZE = (() => {
  const n = Number(process.env.STOCK_BOTTOM_CANDLE_BATCH ?? process.env.STOCK_GOLDEN_CROSS_BATCH ?? 6);
  return Number.isFinite(n) && n >= 1 ? Math.min(n, 12) : 6;
})();

const BATCH_DELAY_MS = (() => {
  const n = Number(
    process.env.STOCK_BOTTOM_CANDLE_BATCH_DELAY_MS ??
...
async function scanOneSymbol(item, market, scanDate, timeframe = "1d") {
  ...
  try {
    ...
    if (!tradable.ok) {
      liveTradeLogInfo("[bottom-candle:scan] skip", sym, tf, tradable.reason);
    ...
    const det = detectBottomCandleLatest(candles, {
      chartIsWeekly: tf === "1wk",
      chartIsDaily: tf === "1d",
    });
    if (!det.anyBottom) return { ok: true, hit: null };
    return {
      ok: true,
      hit: {
        symbol: sym,
        ...
        bottomTag: det.tag,
        bottomCode: det.tagCode,
        bottomScore: det.score,
        bottomSl: det.slPrice,
        bottomZoneTop: det.zoneTop,
        bottomZoneBot: det.zoneBot,
        bottomRvol: det.rvol,
        bottomClassic: det.bottomClassic,
      },
    };
  } catch (e) {
    return finishVaultScanSymbolOnLoadError("bottom-candle", sym, tf, e);
  }
}
...
export async function runBottomCandleMarketScan(market, scanDate, opts = {}) {
  ...
  liveTradeLogInfo("[bottom-candle:scan] start", {
    market,
    scanDate,
    timeframe,
    universe: uni.scope,
