 ...
 *   scanned: number;
 *   hitCount: number;
 *   hits: Array<{ symbol: string; name: string; market: "kr"|"us"; scanDate: string }>;
 * }} MaAlignHistoryEntry
 */
 ...
 *   scanDate: string;
 *   scanned: number;
 *   hits: MaAlignHistoryEntry["hits"];
 * }} input
 */
export function appendMaAlignHistoryEntrySync(input) {
...
export function listMaAlignHistorySync(opts = {}) {
...
/** @param {{ scanDate?: string; limit?: number }} [opts] */
export function listMaAlignHistoryRunsSync(opts = {}) {
  const entries = listMaAlignHistorySync(opts);
  /** @type {Map<string, { runId: string; atMs: number; trigger: "manual"|"scheduled"; markets: MaAlignHistoryEntry[] }>} */
  const byRun = new Map();
...
export function listMaAlignHistoryDatesSync() {
  const dates = new Set(
    readStore()
